...
public class MainActivity extends Activity implements CvCameraViewListener2  {
    ...
    GalleryScanner 				   galleryFragment;
    ...
    public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
        ...
        if (selectedPreprocessor[SKINCOLOUR_THRESHOLD]) {
        	...
        	faceclass.expclass.nativelib.skinThreshold(mRgba, clip, true);
        ...
        if (facesArray.length == 0) { // no faces text
	        ...
	        for (Rect r : facesArray)
	        {
            	...
            	if (r.tl().x < 0) continue;
	        	if (r.tl().y < 0) continue;
	        	if (r.br().x > mGray.width()-1) continue;
	        	if (r.br().y > mGray.height()-1) continue;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item == mItemSystem) {
    		...
    		builder.setTitle("Test triggering");
  ...
  protected void scanGallery() {
        ...
        galleryFragment = GalleryScanner.newInstance(faceclass, mOpenCvCameraView);
        galleryFragment.show(ft, "GalleryScanner");
	}
